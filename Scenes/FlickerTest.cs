...
public class FlickerTest : Scene {
	...
	public void Declarations() {
		VRamQueue			= AL.Module<VRamQueue>();
		_gamepad			= AL.Module<Gamepads>()[0];
	...
	public override void Step() {
		...
		If.True(_gamepad.WasPressed(NES.Button.Start), () => AL.Module<SceneManager>().Queue(AL.Module<Title>()));
		...
		If.Block(c => c
			...
			.Else(() => {
				AL.Comment("Moving scattered objects pattern");
				Y.Set(startOffset);
	...
	private void Skipterate_Next() {
		_index.Set(z => A.Set(AL.LabelFor(SkipOption)[Y.Set(_skipOptionIndex)]).Add(z));
	...
	public void DrawSkipNum() {
		...
		VRamQueue.Push(() => A.Set(AL.LabelFor(SkipOptionTile10s)[X.Set(_skipOptionIndex)]));
		VRamQueue.Push(() => A.Set(AL.LabelFor(SkipOptionTile1s)[X]));
	...
	[DataSection]
	public static void SkipOption() => AL.Raw(_skipOptions);
	[DataSection]
	public static void SkipOptionTile10s() => AL.Raw(_skipOptions.Select(x => _digitTiles[x / 10]).ToArray());
	[DataSection]
	public static void SkipOptionTile1s() => AL.Raw(_skipOptions.Select(x => _digitTiles[x % 10]).ToArray());
	...
	public class ScatterSprite : Struct {
		...
		public void Init() {
			PosX.Set(AL.LabelFor(StartX)[Index]);
			PosY.Set(AL.LabelFor(StartY)[Index]);
		}

		public void Move() {
			PosX.Set(z => A.Set(z).Add(AL.LabelFor(MoveX)[Index]));
			PosY.Set(z => A.Set(z).Add(AL.LabelFor(MoveY)[Index]));
		...
		[DataSection]
		public static void StartX() =>	AL.Raw(133, 92, 50, 105, 161, 32, 9, 118, 246, 176, 143, 42, 19, 3, 240, 24);
		[DataSection]
		public static void StartY() =>	AL.Raw(146, 128, 95, 71, 140, 145, 119, 124, 65, 127, 108, 99, 70, 104, 118, 85);
		[DataSection]
		public static void MoveX() =>	AL.Raw(0xFD, 0xFE, 0xFE, 0x01, 0x02, 0x01, 0xFE, 0xFE, 0x01, 0xFE, 0xFD, 0xFD, 0x02, 0x01, 0xFE, 0xFD);
		[DataSection]
		public static void MoveY() =>	AL.Raw(0xFD, 0x02, 0x01, 0xFE, 0x02, 0xFE, 0x02, 0x01, 0xFD, 0xFD, 0x01, 0x01, 0x02, 0xFE, 0x01, 0xFD);
	}
}
