...
public class Engine : Module {
	...
	public void Reset() {
		...
		AL.Module<SceneManager>().Load(AL.Module<Scenes.Title>());
		Hardware.WaitForVBlank();
		NES.PPU.Control.Set(NES.PPU.LazyControl);
		NES.PPU.Mask.Set(NES.PPU.LazyMask);
		ScrollUtil.Update();
		Loop.Infinite(_ => {
			AL.Comment("Main Loop");

			AL.Module<Gamepads>().Update();

			AL.Module<SceneManager>().Step();

			A.Set(nmiCount);
			Loop.Do_old().While(() => nmiCount.Equals(A) );

			AL.Module<SceneManager>().CheckLoadNeeded();
